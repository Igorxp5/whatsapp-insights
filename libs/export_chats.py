...
CHAT_MESSAGE_PATTERN = re.compile(r'^(?P<date>\d{1,2}\/\d{1,2}\/(?P<year>\d{2}|\d{4}),? \d{1,2}:\d{1,2}) - (?P<contact_name>.+?): (?P<message>.+)', re.MULTILINE)
...
def parse_export_chat_file(filepath: FilePath, tz: datetime.tzinfo=None) -> typing.List[ExportChatMessage]:
    ...
    for i, header in enumerate(message_headers):
        ...
        date = datetime.datetime.strptime(header.group('date'), date_format)
        if tz:
